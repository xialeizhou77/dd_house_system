...
import { AnnotateMarker } from '../components/BuildingMarker';
...
export default function AnnotatePage() {
  ...
  return (
    <div className="annotate-page annotate-page--immersive">
      {/* Full screen map background */}
      <div className="annotate-map-bg" aria-hidden />

      {/* Floating toolbar */}
      <div className="annotate-floating-toolbar glass-panel">
        <button type="button" className="btn-back-glass" onClick={() => navigate(-1)}>
          <IconArrowLeft />
          返回
        </button>
        <button
      ...
      {/* Floating sidebar */}
      <aside className="annotate-sidebar-glass glass-panel">
        <p className="sidebar-hint">点击底图上的位置，填写楼号与区域，即可添加/更新该楼栋坐标。</p>
        <div className="coord-list">
          <h3>当前标注 ({coords.length})</h3>
          {loading ? (
            <p className="coord-loading">加载中...</p>
          ) : (
            <ul>
        ...
        {message && <p className={`annotate-message ${message.includes('失败') ? 'error' : ''}`}>{message}</p>}
      </aside>

      {/* Map area */}
      <div className="annotate-map-wrap">
        <div
          className="annotate-map-full"
        ...
        >
          {coords.map((c) => (
            <AnnotateMarker key={c.id} coord={c} />
          ))}
          {pending && (
            <span
              className="annotate-pin-pending"
      ...
      {pending && (
        <div
          className="annotate-modal-overlay"
          onClick={() => {
            setPending(null);
            setPendingLabel('');
          }}
        >
          <div
            className="annotate-modal-dialog glass-panel"
